...
/**
 * Q1 wk7+ — resolve the render source for clip generation from an
 * Episode's field values, with a backwards-compatible fallback.
 *
 * The wk4 commit populates `Episode.sourceVideoUrl` at ingest time:
 ...
 * and ffmpeg will surface "no video stream" via the wk7 error translator
 * if the file happens to be audio-only.
 *
 * For RSS/PASTE/YOUTUBE sources, we do NOT fall back — those sources
 * either have their own `sourceVideoUrl` populated (YOUTUBE, wk4+) or
