...
/**
 * Minimal `step` surface used by the sync handler. Extracted so unit tests
 * can invoke `syncScheduledOutputsHandler` with a stub that runs the block
 * inline. In production the real Inngest step runtime provides durable
 * checkpointing + error boundaries per named step. Return typed as
 * `Promise<unknown>` because Inngest's real `step.run` returns
 * `Jsonify<Awaited<T>>` — we don't consume the value here.
 */
export type SyncStep = {
  run: (id: string, fn: () => Promise<unknown>) => Promise<unknown>;
};

export type SyncScheduledOutputsResult = {
  scanned: number;
  bufferConfirmed: number;
  bufferSkipped: number;
  bufferFailed: number;
  bufferDowngraded: number;
  manualAutoPublished: number;
  errors: number;
};

export async function syncScheduledOutputsHandler(
  step: SyncStep,
): Promise<SyncScheduledOutputsResult> {
...
export const syncScheduledOutputs = inngest.createFunction(
  ...
  async ({ step }) => syncScheduledOutputsHandler(step),
);
